...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        # Get next_ob predictions from all models in the ensemble and average them
        next_ob_predictions = np.array([
            mb_agent.get_dynamics_predictions(i, ob[None], ac[None])[0]
            for i in range(mb_agent.ensemble_size)
        ])
        next_ob = next_ob_predictions.mean(axis=0)
...
def run_training_loop(
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                # Sample a different batch for each model
