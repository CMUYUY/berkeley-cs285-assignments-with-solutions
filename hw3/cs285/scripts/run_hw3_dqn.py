    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(
                action=action,
                reward=reward,
                next_observation=next_observation[-1, ...],  # Only last frame
        ...
        if step >= config["learning_starts"]:
            ...
            # where each array has shape (batch_size, ...).
            # `agent.update` should return a dictionary of scalars, which will be logged.
